...
export const StoreApi = api.injectEndpoints({
  endpoints: (builder) => ({
    storess: builder.query<
      any,
      { page: number; perPage: number; id: any; role: string; subName?: string }
    >({
      query: ({ page, perPage, role, id, subName }) => {
        if (role === "VENDOR") {
          // If role is VENDOR, search by vendor_id and optional subName
          return `api/stores/my-stores?page=${page}&perPage=${perPage}&vendor_id=${id}&${
            subName ? `name=${subName}` : ""
          }`;
        } else {
          // If role is not VENDOR, search by name only
          return `api/stores?page=${page}&perPage=${perPage}&${
            subName ? `name=${subName}` : ""
          }`;
        }
      },
    ...
    stores: builder.query<
      any,
      { page: Number; perPage: Number; id: any; role: string }
    >({
      query: ({ page, perPage, role, id }) =>
        `api/stores?page=${page}&perPage=${perPage}&${
          role === "VENDOR" ? `?vendor_id=${id} ;` : ""
        }`,
      providesTags: ["stores"],
    }),
    ...
    searchStores: builder.query<
      any,
      { subName: string; role: string; id: string }
    >({
      query: ({ subName, role, id }) =>
        `api/stores?search=name:${subName}${
          role === "VENDOR" ? `;vendor_id:${id};` : ""
        }`,
