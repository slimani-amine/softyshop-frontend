...
export const VendorsApi = api.injectEndpoints({
  endpoints: (builder) => ({
    ...
    vendors: builder.query<any, { perPage: number; page: number }>({
      query: ({perPage , page}) => `api/users?perPage=${perPage}&page=${page}&search=role:vendor`,
...
export const {useAllvendorsQuery,useCreateVendorMutation,useDeleteVendorMutation,useVendorQuery,useVendorsQuery,useSearchVendorsQuery,useUpdateVendorMutation} = VendorsApi