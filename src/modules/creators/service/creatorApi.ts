...
export const CreatorApi = api.injectEndpoints({
  endpoints: (builder) => ({
    creators: builder.query<any,  { perPage: number; page: number }>({
      query: ({perPage,page}) => `api/productCreators?perPage=${perPage}&page=${page}`,
...
export const {useCreatorsQuery,useAllCreatorsQuery, useSearchCreatorQuery,useUpdateCreatorMutation,useDeleteCreatorMutation ,useDeleteCreatorsMutation,useCreateCreatorMutation,useCreatorQuery} = CreatorApi